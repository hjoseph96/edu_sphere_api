...
module Api
  module V1
    class DocumentsController < ApplicationController
      def index
        documents = if current_user.role == 'teacher'
        ...
        render json: { documents: documents.map(&:attributes),
                       shared_documents: current_user.shared_documents.map(&:attributes) }
      end
      ...
      def create
        documents = document_params[:files].to_h.map do |_i, document_data|
      ...
      def update
        ...
        if document.update(document_update_params)
          ...
          render json: { document: document.attributes.merge(versions: document.filtered_versions.reverse[0..9]) },
                 status: :ok
        else
      ...
      def document_params
        params.require(:documents).permit(files: %i[title file])
      end
      ...
      def document_editors_params
        params.require(:document_editors).permit(editors: %i[user_id role])
      end
    end
  end
end
